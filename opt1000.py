...
RIVET_DICT = {4: 3.175, 5: 3.96875}  # リベット径[mm]の候補辞書(keyは呼び番号)
DIVISION_LIST = np.array([3, 4])  # webの分割数の候補リスト
STIFFENER_THICKNESS_LIST = np.array([1.80, 2.03, 2.29, 2.54, 3.18])  # 適当
FLANGE_THICKNESS_LIST = np.array([7 + i for i in range(5)])  # 適当
STIFFENER_B_LIST = np.array([20 + i for i in range(20)])
FLANGE_B_LIST = np.array([30 + i for i in range(15)])
...
def make_sta1000():
    min_mass = 100  # [kg]
    counter = 0
    n = len(WEB_THICKNESS_LIST) * len(STIFFENER_THICKNESS_LIST) * len(FLANGE_THICKNESS_LIST) ** 2 * \
