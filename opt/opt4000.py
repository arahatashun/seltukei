...
RIVET_DICT = {4: 3.175, 5: 3.96875, 6: 4.7625}  # リベット径[mm]の候補辞書(keyは呼び番号)
DIVISION_LIST = [3]  # webの分割数の候補リスト
# STIFFENER_THICKNESS_LIST = [1.80, 2.03, 2.29, 2.54, 3.18]  # 適当
FLANGE_THICKNESS_LIST = [2 + i * 0.5 for i in range(10)]  # 適当
STIFFENER_B_LIST = [6 + i for i in range(10)]
STIFFENER_B_BOTTOM_LIST = [12 + i for i in range(5)]
FLANGE_B_LIST = [10 + i for i in range(10)]
                                    ...
                                    for div in DIVISION_LIST:
                                        for D1 in RIVET_DICT.values():
                                            for D2 in RIVET_DICT.values():
                                                sta4000 = Rib(7)
                                                sta4000.add_web(1.60, div)
                                                sta4000.add_stiffener(1.80, bs1, bs2)
                                                sta4000.add_compression_flange(fc_thickness, bf1, bf2)
                                                sta4000.add_tension_flange(ft_thickness, bf3, bf4)
                                                sta4000.add_rivet_stiffener(D1)
                                                sta4000.add_rivet_flange(D2, 6, 2)
                                                sta4000.set_he()
                                                if sta4000.decide_ms() == True:
                                                    mass = sta4000.get_total_mass()  # [kg]
                                                    if mass < min_mass:
                                                        mass_csv([mass])
                                                        counter += 1
                                                        min_mass = mass
                                                        ...
                                                        sta4000.tflange_csv()
                                                        sta4000.cflange_csv()
                                                        sta4000.stiffener_csv()
                                                        sta4000.rivet_stiffener_csv()
                                                        sta4000.rivet_flange_csv()
                                                    else:
                                                        counter += 1
                                                        print("counter: {0}/{1}\n".format(counter, n))
                                                else:
