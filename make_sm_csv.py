...
HARF_SPAN = 5000  # [mm]
Y_REP_FOR_C = 5000 * \
    np.array([0, 0.2, 0.4, 0.6, 0.8, 0.9, 0.95, 0.975, 1.0])  # [mm]
...
Y_DISTANCE_FOR_W = np.array([Y_REP_FOR_W[i + 1] - Y_REP_FOR_W[i]
                             for i in range(0, Y_REP_FOR_W.shape[0] - 1)])
...
def getCCzIntegral(y):
    step = 1
    ans = sum([getChoord(y + step * i) * getCZ(y + step * i) *
               step for i in np.arange(np.floor((HARF_SPAN - y) / step))])
